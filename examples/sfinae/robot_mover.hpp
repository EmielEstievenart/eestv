...
template <typename, typename = void> struct has_member_move : std::false_type
{
};

template <typename T> struct has_member_move<T, std::void_t<decltype(std::declval<T&>().move())> > : std::true_type
{
};

// Trait: has a free function `move(T&)` found by ADL (unqualified call)
template <typename, typename = void> struct has_adl_move : std::false_type
{
};

template <typename T> struct has_adl_move<T, std::void_t<decltype(move(std::declval<T&>()))> > : std::true_type
{
};
...
template <typename RobotType> void move_the_robot(RobotType& robot)
{
