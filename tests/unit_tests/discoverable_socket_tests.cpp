...
TEST_F(DiscoverableTcpSocketTest, DiscoveryAndConnection) {
    ...
    discovering.async_connect_via_discovery([&connect_promise](boost::system::error_code ec) {
        connect_promise.set_value(ec);
    });
    
    // Run the IO context briefly to handle the connection
    std::thread connect_io_thread([this]() {
    ...
    discoverable.async_accept(accepted_socket, [&accept_promise](boost::system::error_code ec) {
