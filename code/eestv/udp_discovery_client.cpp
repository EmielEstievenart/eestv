...
UdpDiscoveryClient::UdpDiscoveryClient(
    boost::asio::io_context &io_context, const std::string &service_name,
    std::chrono::milliseconds retry_timeout, int port,
    std::function<bool(const std::string &)> response_handler)
    : _io_context(io_context), _service_name(service_name),
      _socket(io_context, boost::asio::ip::udp::v4()),
      _retry_timeout(retry_timeout), _timer(io_context), _running(false),
      _port(port), _response_handler(std::move(response_handler)) {
  _socket.set_option(boost::asio::ip::multicast::outbound_interface(
...
void UdpDiscoveryClient::_send_discovery_request() {
  ...
  boost::asio::ip::udp::endpoint broadcast_endpoint(
      boost::asio::ip::make_address("255.255.255.255"), _port);
  ...
  _socket.async_send_to(boost::asio::buffer(_service_name), broadcast_endpoint,
                        [this](const boost::system::error_code &error,
                          ...
                          if (error) {
                            std::cerr << "Failed to send discovery request: "
...
void UdpDiscoveryClient::_handle_response(
  ...
  if (!error && bytes_transferred > 0) {
    ...
    std::cout << "Discovery response from " << _remote_endpoint << ": "
              << response << std::endl;
    ...
    std::cerr << "Error receiving discovery response: " << error.message()
              << std::endl;
  }
}
